...
consumer = k.create_consumer("alert-group", k.topic1)
...
if __name__ == "__main__":
    ...
    metrics.FLIGHTS_DOWNLOAD_DATA.labels(service='datacollector', node=db.NODE_NAME, resource='flight_departure').set(0)
    metrics.FLIGHTS_DOWNLOAD_DATA.labels(service='datacollector', node=db.NODE_NAME, resource='flight_arrive').set(0)
    metrics.FLIGHTS_DOWNLOAD_DATA.labels(service='datacollector', node=db.NODE_NAME, resource='total_time').set(0)
    metrics.FLIGHTS_DOWNLOAD_DATA.labels(service='datacollector', node=db.NODE_NAME, resource='last_n_voli').set(0)
    metrics.NUM_FLIGHTS_DOWNLOADED.labels(service='datacollector', node=db.NODE_NAME, resource='total_n_voli').inc(0)
